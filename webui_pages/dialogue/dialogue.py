...
def dialogue_public_page(api: ApiRequest):
    chat_box.init_session()
    ...
    with st.sidebar:
        ...
        if dialogue_mode == "知识库问答":
            logger.info("进入知识库问答模式......")
            with st.expander("知识库配置", True):
                kb_list = api.list_knowledge_bases(no_remote_api=True)
                selected_kb = "social-governance"
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        ...
        if dialogue_mode == "LLM 对话":
            ...
            chat_box.ai_say([
                ...
                "正在思考...", 
                # Markdown("...", in_expander=True, title="知识库匹配结果"),
                Markdown("", in_expander=False, title=""),
            ])
            text = ""
            once_print = True
            for d in api.knowledge_base_chat(prompt, selected_kb, kb_top_k, score_threshold, history, model=llm_model):
                if error_msg := check_error_msg(d): # check whether error occured
                    ...
                    if once_print:
                        ref_docs_str = "\n\n".join(d["docs"])
                        logger.info(f"参考知识库：{ref_docs_str}")
                        once_print = False
            chat_box.update_msg(text, 0, streaming=False)
        elif dialogue_mode == "搜索引擎问答":
            chat_box.ai_say([
