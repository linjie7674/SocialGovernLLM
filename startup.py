...
def run_webui(q: Queue, run_seq: int = 5, is_public: bool = False):
    ...
    if q is not None and isinstance(run_seq, int):
        while True:
            ...
            if no != run_seq - 1:
                ...
                break
        q.put(run_seq)
    p = subprocess.Popen(["streamlit", "run", "webui_public.py" if is_public else "webui.py",
                          "--server.address", host,
...
async def start_main_server():
    ...
    if args.webui:
        ...
        process = Process(
            target=run_webui,
            name=f"WEBUI Server",
            args=(queue, process_count() + 1, args.is_public),
            daemon=True,
        )
